...
namespace OneDayOneDev_DayEight
{
    public class FileHandler
    {
        ...
        public OperationResult SaveTaskExport(List<TaskItem> Tasks,MenuInfo TypeOfExport)
        {
            ...
            File.AppendAllText(exportPath, "Id;Title;CreatedAt;DueDate;OverDate;IsCompleted\n");
            File.AppendAllLines(exportPath,
                Tasks.Select(t => $"{t.id};{t.Title};{(t.CreatedAt == null ? "" : t.CreatedAt?.ToString("dd/MM/yyyy"))};{(t.DueDate == null ? "" : t.DueDate?.ToString("dd/MM/yyyy"))};{(t.OverDate == null ? "" : t.OverDate?.ToString("dd/MM/yyyy"))};{t.Iscompleted}"));
