...
namespace OneDayOneDev_DayThirten
{
    ...
    public class TaskService
    {
       ...
       public OperationResult ExportToCSV(List<TaskItem> TaskToExport,MenuInfo TypeOfExport)
       {
        ...
        public bool IsTaskLate(TaskItem task)
        {
            ...
            if (task.DueDate >= _DateTime.Today ) return false;
            
            if(task.DueDate < _DateTime.Today) return true;

            return false;
        }

        public bool CanBeDeleted(TaskItem task)
        {
            if (task.Iscompleted || IsTaskLate(task)) return false;
            return true;
        }
